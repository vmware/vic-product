...
#set Harbor
url=$(gsutil ls -l "gs://harbor-builds" | grep "harbor-offline-installer" | grep -v TOTAL | grep -v latest | grep offline-installer | sort -k2 -r | (trap '' PIPE; head -n1) | xargs | cut -d ' ' -f 3 | sed 's/gs:\/\//https:\/\/storage.googleapis.com\//')
setenv HARBOR "$url"
