...
Download CA Cert
    ...
    :FOR  ${IDX}  IN RANGE  5
    \   ${rc}  ${output}=  Run And Return Rc And Output  sshpass -p ${OVA_PASSWORD_ROOT} scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${OVA_USERNAME_ROOT}@%{OVA_IP}:${OVA_CERT_PATH}/ca.crt ${download-path}
...
Setup CA Cert for Harbor Registry
    Download CA Cert
    Run command and Return output  mkdir -p /etc/docker/certs.d/%{OVA_IP}
    Run command and Return output  mv ca.crt /etc/docker/certs.d/%{OVA_IP}/ca.crt
    Run command and Return output  ls /etc/docker/certs.d/%{OVA_IP}
...
Push Docker Image To Harbor Registry
    ...
    Setup CA Cert for Harbor Registry
    Docker Login To Harbor Registry  ${registry-ip}
