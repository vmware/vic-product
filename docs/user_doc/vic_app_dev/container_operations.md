...
Since 1.3 `docker rm -v` is also supported. Running the command removes the container and any anonymous
volumes joined. If an anonymous volume is in use by another container, it is not removed. Named volumes (specified by name in the create/run command) are not deleted.|
