...
- If the NFS is set to `no_squash_root` then the `root` user or group needs permissions on the NFS sharepoint. 
- If `squash_root` is enabled, then the `anon` user or group must have permissions on the NFS sharepoint. 
