...
Image layers are stored in memory during `docker pull`. If you have enabled or inherited memory usage alerts on the VCH endpoint and you download a large VMFS virtual disk (VMDK), you might trigger a memory usage alert. You might want to reconfigure the alerts to avoid receving warnings related to size.
