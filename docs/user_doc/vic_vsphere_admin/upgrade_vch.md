...
- By default, vSphere Integrated Containers 1.2.x deployed VCHs as vApps, unless you explicitly deployed them as resource pools by using the `vic-machine create --use-rp` option. Versions of vSphere Integrated Containers 1.3.x and later always deploy VCHs as resource pools, rather than as vApps. If you upgrade vApp VCHs from version 1.2.x to version 1.4.x, these VCHs remain vApps after the upgrade. Any new container VMs that are deployed after the upgrade are deployed in the existing vApp. It is recommended to delete any remaining vApp VCHs from version 1.2.x and recreate them as version 1.4.x VCHs rather than upgrading them. Any container VMs that were provisioned in the vApp VCH should be recreated as version 1.4.x container VMs in the new version 1.4.x VCH.
