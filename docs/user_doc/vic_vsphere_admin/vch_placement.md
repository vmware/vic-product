...
When deploying a VCH to vCenter Server, you must use the [`--bridge-network`](bridge_network.md) option to specify an existing port group for container VMs to use to communicate with each other. For information about how to create a vSphere Distributed Switch and port group, see the section on vCenter Server Network Requirements in [Environment Prerequisites for VCH Deployment](vic_installation_prereqs.md#networkreqs).
