...
This topic describes the different certificate options that you use when deploying VCHs, and provides examples of how to combine the options to achieve different configurations.
...
You can deploy a VCH to use custom certificates in combination with auto-generated certificates, as demonstrated in [Examples](#examples).
...
2. Enter the IP address, FQDN, or a domain wildcard in the **Common Name (CN)** text box.

#### vic-machine Option 

`--tls-cname`, no short name

The IP address, FQDN, or a domain wildcard, for the client system or systems that connect to this VCH. If you specify `--tls-cname`, `vic-machine create` performs the following actions during the deployment of the VCH:
...
1. For **Source of certificates**, select the **Auto-generate** radio button.
2. Leave the default setting of the VCH name, or enter a different organization identifier.
...
1. For **Source of certificates**, select the **Auto-generate** radio button.
2. Leave the default setting of 2048 bits, or enter a higher value.
...
A custom X.509 server certificate for the VCH if you do not use  an automatically generated server certificate. This certificate identifies the VCH endpoint VM both to Docker clients and to browsers that connect to the VCH Admin portal.
...
If you do not use  an automatically generated server certificate, use this option in combination with the `--tls-server-key` option, that provides the path to the private key file for the custom server certificate. Include the name of the certificate file in the path.
...
- If no existing `server-cert.pem`, `server-key.pem`, or `ca.pem` certificates are present in the folder, `vic-machine` automatically generates them and saves them in the certificate folder.
...
- Finds the custom client certificate in the certificate folder, so does not generate a client certificate. **CHECK THIS**
