# vSphere Integrated Containers Quick Start Links #

If you want to start experimenting with vSphere Integrated Containers straight away, see the following topics:

- [Overview of vSphere Integrated Containers](vic_overview/README.md)
- [Deploy the vSphere Integrated Containers Appliance](vic_vsphere_admin/deploy_vic_appliance.md)
- [Deploy Virtual Container Hosts](vic_vsphere_admin/deploy_vch.md)
- [Index of `vic-machine create` Options](vic_vsphere_admin/vicmachine_create_options_ref.md)
- [Initial Configuration of the Management Portal](vic_cloud_admin/mp_initial_config.md)
- [Create a Project in vSphere Integrated Containers Managment Portal](vic_cloud_admin/create_projects.md)
- [Add Global Registries](vic_cloud_admin/add_repos_in_portal.md)
- [Supported Docker Commands](vic_app_dev/container_operations.md)